...
<section class="mala-content">
        <h1 class="mala-title-bar">图片内容管理</h1>
        <ul class="mala-tab-navi-bar mala-clearfix">
            <li class="mala-tab-navi-item mala-fl mala-mr active">所有图片</li>
        </ul>
        <div class="mala-container-fluid">
					...
					<tbody id='element_list'>
						@foreach($imagelist as $image)
                         <tr class="mala-tr">
                            ...
                            <td class="mala-td"><a href="./mediadetail?Content_ID={{$image['Content_ID']}}" class="mala-mr js-edit">编辑</a> <a href="/api/delmaterials?Content_ID={{$image['Content_ID']}}" class="mala-mr js-edit">删除</a>  <a class="mala-mr js-edit" href="/admin/imagepreview?ContentID={{$image['Content_ID']}}">查看图片</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="mala-pager">
                ...
                <span class="mala-paginate-link">
                    <a href="image?Page={{$currentpage-1}}"><span class="mala-caret mala-caret-prev"></span></a>                    
                </span>
                @endif

                @for ($i = 1; $i < $totalpage+1; $i++)
                	@if($i == $currentpage)
                	<a href="image?Page={{$i}}" class="mala-paginate-link active">{{$i}}</a>
                	@else
                	<a href="image?Page={{$i}}" class="mala-paginate-link">{{$i}}</a>
                	@endif
                @endfor
                 
                @if ($currentpage < $totalpage)  
                <span class="mala-paginate-link">
                    <a href="image?Page={{$currentpage+1}}"><span class="mala-caret mala-caret-next" ></span></a>
                </span>
                @endif
            </div>
        </div>
        ...
        <div class="mala-modal-dialog">
            <div class="mala-modal-content">
                <div class="mala-modal-header">
                    ...
                    <h4 class="mala-modal-title" id="myModalLabel">新增图片</h4>
                </div>
						...
						<div class="mala-container-fluid">
							<div class="mala-form">
								...
								<div class="mala-form-item mala-row">
									<div class="mala-col-md-3">
										<label for="" class="mala-label mala-form-fix">
											<span class="mala-required">*</span>上传图片</label>
									</div>
